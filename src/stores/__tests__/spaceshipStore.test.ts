...
import { planHohmannTransfer } from '../../engine/navigation';
...
function makePlan(dest: string): { phases: NavigationPhase[]; destinationId: string } {
  const aEarth = REAL_DATA.earth.semiMajorAxis!;
  const vEarth = Math.sqrt(MU_SUN_AU / aEarth);
  return planHohmannTransfer([aEarth, 0, 0], [0, vEarth, 0], dest, Date.now());
}
...
describe('spaceshipStore navigation lifecycle', () => {
  ...
  it('merge should be called after sub-step completion regenerates remaining', () => {
    useSpaceshipStore.getState().setTargetBody('mars');
    let s = useSpaceshipStore.getState();
    const plan = s.navigationPlan!;
    ...
    useSpaceshipStore.getState().checkNavigationalDeviation();
    s = useSpaceshipStore.getState();
    // Remaining regenerated by generatePhaseNextSubSteps → phase 1 name triggers burn_prograde/retrograde
