...
**UI 设计**：浮动在画布右上角的 3×3 方向键网格（↑←↻→↓），半透明深色背景 + 毛玻璃效果（backdrop-filter: blur）。

**交互方式**：
- 按住方向键通过 setInterval（50ms 间隔）持续旋转相机，步长为 CAMERA_ROTATE_STEP(π/18) × 0.5
- 松开鼠标/鼠标离开停止旋转
- 中间 ↻ 按钮重置相机视角
- 不支持滚轮缩放、不支持画布拖拽旋转——这是明确的设计决策，用户只能通过方向键旋转视角

**关键决策**：
- **为什么不支持滚轮缩放和画布拖拽**：教育工具面向非技术用户，旋转/缩放操作容易导致迷航。方向键提供可预测的受控视角变化
- **为什么通过 cameraRef 共享相机**：CameraControls 需要操作相机但不应持有 Canvas3D 的引用，全局引用是最简单的解耦方案
