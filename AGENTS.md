...
1. **任何产品功能变更必须先更新设计文档** (`docs/specs/` 或 `docs/plans/`)，然后再修改代码。不允许只改产品代码不更新文档。
...
用于验证页面操作流程、物理模拟结果、导航策略、性能或日志分析的辅助脚本，可以不新增或更新 `docs/specs/`、`docs/plans/`，前提是：

1. 脚本不改变产品运行时代码的功能、接口或用户可见行为。
2. 脚本只位于 `scripts/` 或类似辅助目录中，用于本地验证、分析或复现实验。
3. 脚本文件顶部必须写清楚背景、验证目标、运行方式、输入输出、成功判定和日志位置。
4. 如果脚本验证过程中发现需要修改 `src/` 下的产品功能、接口、状态结构或用户行为，则仍需先更新对应设计文档，再修改产品代码。
5. 如果脚本本身沉淀为长期维护的开发工具，应在脚本注释或 README/AGENTS 中说明其职责和使用方式。
